...
> Create sandboxed execution environments with a unified API across Vercel, Cloudflare, and Deno Deploy.
...
interface Sandbox {
  readonly id: string
  readonly provider: 'vercel' | 'cloudflare' | 'deno'
...
See [sandbox-vercel](/modules/providers/sandbox-vercel), [sandbox-cloudflare](/modules/providers/sandbox-cloudflare), and [sandbox-deno](/modules/providers/sandbox-deno) for provider-specific documentation.
...
// Typed as DenoSandbox
const deno = await createSandbox({
  provider: { name: 'deno' },
})
await deno.deno.exposeHttp({ port: 8080 }) // ✓ typed
```
...
| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `name` | `'deno'` | - | Provider name |
| `allowNet` | `string[]` | - | Allowed network hosts |
| `debug` | `boolean` | - | Enable debug output |
| `env` | `Record<string, string>` | - | Environment variables |
| `labels` | `Record<string, string>` | - | Sandbox labels |
| `memory` | `string \| number` | - | Memory limit |
| `port` | `number` | - | Default port |
| `region` | `string` | - | Sandbox region |
| `root` | `string` | - | Working directory |
| `sandboxEndpoint` | `string` | - | Custom API endpoint |
| `secrets` | `Record<string, { hosts: string[], value: string }>` | - | Secret injection |
| `ssh` | `boolean` | - | Enable SSH access |
| `timeout` | `` `${number}s` \| `${number}m` \| 'session' `` | - | Timeout |
| `volumes` | `Record<string, string>` | - | Mount volumes |
